<?php
/**
 * ThinkPHP [ WE CAN DO IT JUST THINK ] 微信支付插件
 +----------------------------------------------------------------------
...
{    
    ...
    public function  __construct() {   
        ...
        require_once("example/WxPay.BusinessPay.php");
    ...
    {       
            ...
            return '<img alt="模式二扫码支付" src="/index.php?m=Home&c=Index&a=qr_code&data='.urlencode($url2).'" style="width:110px;height:110px;"/>';    
             // return '/index.php?m=Home&c=Index&a=qr_code&data='.urlencode($url2);       
    }    
    /**
     * 服务器点对点响应操作给支付接口方调用
     * 
    ...
    function getJSAPI($order){
        ...
        }elseif (stripos($order['order_sn'],'partnerP') !== false) {
...
EOF;
    ...
    public function businessPay($data=array())
    {
        $bPay = new BusinessPay();
        $values['nonce_str']  =$data['nonce_str'];
        $values['partner_trade_no']  = $data['partner_trade_no'];
        $values['openid']  = $data['openid'];
        $values['check_name']  ='NO_CHECK';
        $values['amount'] =$data['amount']*100;
        $values['desc']  =$data['desc'];
        $values['spbill_create_ip']  =$data['ip'];
        $bPay->setValues($values);
        $res = $bPay->curl_post_ssl();
        return $res;
