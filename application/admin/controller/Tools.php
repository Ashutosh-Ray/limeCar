...
//        send_http_status('310');
        ...
        $path = UPLOAD_PATH."sqldata/imshop_tables_" . date("Ymd").get_rand_str(3,0);
        ...
        foreach ($tables as $table) {
            ...
            for ($i = 1; $i <= $page; $i++) {
                ...
                foreach ($query as $val) {     
                    ...
                    $path = UPLOAD_PATH."sqldata/imshop_{$table}_" . date("Ymd").get_rand_str(3,0);
    ...
    private function getRestoreFiles() {
    	$sqlfilepre = I('sqlfilepre','');//获取sql文件前缀 imshop_tp_article_cat_20170206pno_4.sql.html
    	if (empty($sqlfilepre)) {
