...
class System extends Base{
    ...
    public function navigationList(){
           ...
           $navigationList = $model->where($where)->order("id desc")->select();            
    ...
    public function addEditNav()
    {
        ...
        foreach ($cat_list AS $key => $value) {
            ...
            // $select_val = "/index.php/Home/Goods/goodsList/id/$key";
            $select_option[$select_val] = str_pad('', $strpad_count, "-", STR_PAD_LEFT) . $value['name'].'（电脑端）';
        }
        ...
        $system_nav = array(
            ...
            '/index.php?m=Mobile&c=Goods&a=shops' => '店铺（微信端）',
        );
        ...
        foreach ($cat_list AS $key => $value) {
            ...
            // $select_val = "/index.php/Home/Goods/goodsList/id/$key";
            $select_option[$select_val] = str_pad('', $strpad_count, "-", STR_PAD_LEFT) . $value['name'].'（微信端）';
        }
    }
...
//                exit;
            ...
            if (IS_AJAX) {
                exit(json_encode(array('status'=>1,'msg'=>'清除成功'))); 
            }else{
             $this->success("操作完成!!!",U('Admin/System/navigationList'));
            exit();   
            }
     ...
     */        
     ...
     function ajax_get_action()
     {
        ...
        $advContrl = get_class_methods("app\\admin\\controller\\".str_replace('Controller','',$control));
        //dump($advContrl);
     ...
     public function edit_right(){
        if(IS_POST){
            ...
            $data = I('post.');
            $str = implode(',',$data['right']);
           $data['right']= str_replace('.php','Controller',$str);
        ...
        while($dir = readdir($dirRes))
        {
            ...
            {
                $str = basename($dir,'.class.php');
                $planList[] = str_replace('.php','',$str).'Controller';
