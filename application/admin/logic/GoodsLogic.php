...
class GoodsLogic extends Model
{
     ...
     * @return  mix
     */
    public function goods_cat_list($where='',$cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
    ...
    {  
         $attributeModel = M('AttrVal');
         $attributeModel->where(array('goods_id'=>$goods_id))->delete();
         $attr_ids =  $data['attribute_ids'];
         $attr_vals = $data['attr_vals'];
         if(!is_array( $attr_ids)) return false;
         foreach ($attr_ids as $key => $attr_id) {
                      $save_data = array();
                      $save_data['goods_id'] = $goods_id;
                      $save_data['attribute_id'] = $attr_id;
                     
                      if (!empty($attr_vals[$key])) {
                           $save_data['attr_val'] = $attr_vals[$key];
                           $attributeModel->add($save_data);
                      }
                   }          
    ...
    *保存指定消费时间
    */
    public function savaConsumeTime($goods_id,$data)
    {
       
        M('ConsumeTime')->where(array('goods_id'=>$goods_id))->delete();
        $start_time = strtotime($data['start_time']);
        $end_time = strtotime($data['end_time']);
        $saveData = array();
        if ($start_time>=$end_time) {
            $saveData['start_time'] = $end_time;
            $saveData['end_time'] =  $start_time;
        }else{
            $saveData['start_time'] = $start_time;
            $saveData['end_time'] =  $end_time;

        }

        $saveData['goods_id'] = $goods_id;
    
        M('ConsumeTime')->add($saveData);
    }


    public function savaUsedShops($goods_id,$data)
    {
        
        M('Guseb')->where(array('goods_id'=>$goods_id))->delete();
        ...
        if (is_array($shops)) {
            foreach ($shops as $key => $value) {
               $saveData = array();
               $saveData['goods_id'] = $goods_id;
               $saveData['shop_id'] = $value;
               M('Guseb')->add($saveData);
            }
        }
     ...
     * 
     */
    public function getUsedBusiness($goods_id)
    {
      $gshop =   M('Guseb')->where(array('goods_id'=>$goods_id))->field('shop_id')->select();
      $allShop = M('Admin')->where(array('type'=>1,'status'=>1,'check_status'=>1,'del_status'=>0))->field('admin_id,shop_name')->select();
      if (is_array($gshop)) {
          foreach ($gshop as $g => $gvalue) {
              foreach ($allShop as $a => $avalue) {
                  if ($avalue['admin_id']==$gvalue['shop_id']) {
                      $avalue['selected'] = 1;
                      $allShop[$a] = $avalue;
                  }
              }
    ...
    */
     ...
     public function saveGoodsProgramme($goods_id,$data)
     {
        M('GoodsProgramme')->where(array('goods_id'=>$goods_id))->delete();
        $down_payments = $data['down_payments'];
        $first_year  = $data['first_year'];
        $residual_value  = $data['residual_value'];
        $tail_periods  = $data['tail_periods'];
        $tail_money  = $data['tail_money'];
        if (!is_array($down_payments)) return false;
       
        foreach ($down_payments as $key => $val) {
            $addData = array();
            $addData['goods_id']=$goods_id;
            $addData['down_payments']= $val;
            isset($first_year[$key])?$addData['first_year']=$first_year[$key]:$addData['first_year']=0;
            isset($residual_value[$key])?$addData['residual_value']=$residual_value[$key]:$addData['residual_value']=0;
            isset($tail_periods[$key])?$addData['tail_periods']=$tail_periods[$key]:$addData['tail_periods']=0;
            isset($tail_money[$key])?$addData['tail_money']=$tail_money[$key]:$addData['tail_money']=0;
            M('GoodsProgramme')->save($addData);
        }
     }
    ...
    */
     ...
     public function saveGoodsTags($goods_id,$data)
     {
        M('GoodsTag')->where(array('goods_id'=>$goods_id))->delete();
        $tags = $data['tags'];
    ...
    */
     ...
     public function saveGoodsUserLevel($goods_id,$data)
     {
        M('GoodsLevel')->where(array('goods_id'=>$goods_id))->delete();
        $levels = $data['levels'];
    ...
    */
     ...
     public function getGoodsProgramme($goods_id)
     {
       $data = M('GoodsProgramme')->where(array('goods_id'=>$goods_id))->select();
       return $data;
     ...
     *  获取排好序的品牌列表    
     */
    function getSortBrands($where='')
    {

        $brandList =  M("Brand")->where($where)->select();
        ...
        if (!empty($where)) {
             $brandIdArr =  M("Brand")->where("$where AND name in (select `name` from `".C('database.prefix')."brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
             $goodsCategoryArr = M('goodsCategory')->where("$where AND level = 1")->getField('id,name');
        }
     ...
     *  获取排好序的分类列表     
     */
    function getSortCategory($where='')
    {
        $categoryList =  M("GoodsCategory")->where($where)->getField('id,name,parent_id,level');
        $nameList = array();
