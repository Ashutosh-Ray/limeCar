...
class User extends MobileBase
{
    ...
    */
    ...
    public function index()
    {
        ...
        $coupon_count = M('coupon_list')->where(array("uid"=>$this->user_id,'user_del'=>0,'order_id'=>0))->count(); // 我的优惠券数量
...
//            header("Location: " . U('Mobile/User/index'));
    ...
    public function order_list()
    {
       ...
       if(I('get.check_status')!=''){
        ...
        $this->assign('check_status',I('get.check_status'));
    ...
    */
    public function useBusiness()
    {
        ...
        $lon1 = I('get.lon');
        $rec_id = I('rec_id/d',0);
        if(!is_numeric($rec_id)||empty($rec_id)) $this->error('无可用商家');
        $order_goods = M('OrderGoods')->where(array('rec_id'=>$rec_id))->find();
        if (!is_array($order_goods)) $this->error('订单不包括该商品');
        if ($order_goods['is_appoint']==0) {
           $this->redirect('Mobile/Cart/useBusiness',array('lat'=>$lat1,'lon'=>$lon1));
        }else{
            $data = M('Oguseb')->alias('og')->join('__ADMIN__ a','a.admin_id=og.shop_id')->where(array('og.rec_id'=>$rec_id))->select();
            $list = array();
            foreach ($data as $key => $value) {
                $distance=getDistance($lat1,$lon1,$value['shop_lat'],$value['shop_lon']);
                $value['distance'] =number_format($distance/1000,2) ;
                ...
                return $this->fetch('Cart/useBusiness');
        }
     }
...
//会员卡
    ...
    {
        $mobile_validated = $_SESSION['user']['mobile_validated'];
        $user = $_SESSION['user'];
        if($mobile_validated!=1) $this->error('请认证手机号',U('/Mobile/Distribut/apply'));
        $data = M('card')->alias('c')->join('__CARD_TYPE__ t','t.id = c.card_type')->where('c.host_phone='.$user['mobile'])->field('c.*,t.card_name')->select();
        // $cards = M('card')->where(array('host_phone'=>$user['mobile']))->select();
        // var_dump($data);die();
    ...
    public function card_info()
    {
        ...
        if(!is_array($old)) $this->error('该会员卡不存在');
         $meals = M('CardMeal')->where(array('card_id'=>$card_id))->select();

         $this->assign('meals',$meals);
         return $this->fetch();
    }

    public function consum_log()
    {
        $id = $_GET['id'];
          if(empty($id)||!is_numeric($id)) $this->error('该消费不存在');
        $data= M('Card_log')->alias('l')->join('__ADMIN__ t','t.admin_id = l.op_id')->where('l.card_meal_id='.$id)->field('l.*,t.shop_name')->select();
       if(!is_array($data)) $this->error('该消费不存在');
    ...
    //我的分销商
    public function sharePeople()
    {
       $user_id = $_SESSION['user']['user_id'];
       if (empty($user_id)||!is_numeric($user_id)) return;
        $count = M('share')->alias('s')->join('__USERS__ u','u.user_id=s.second_id')->where('s.first_id='.$user_id)->count();
        $Page = new AjaxPage($count,'10');
        $data =  M('share')->alias('s')->join('__USERS__ u','u.user_id=s.second_id')->where('s.first_id='.$user_id)->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('data',$data);
        if (IS_AJAX) {
           return $this->fetch('ajaxShare');
        }
        return $this->fetch();
    ...
    public function suggest()
    {
        if (IS_POST) {
            ...
            $count =  M('suggestion')->where($map)->count();
            if ($count>=10) {
              exit(json_encode(array('status'=>0,'msg'=>'每天建议不能超过10条')));
            }
            $data = I('post.');
            $data['user_id'] = $this->user_id;
            $data['ip'] = getIP();
           ...
           if ($res) {
              exit(json_encode(array('status'=>1,'msg'=>'感谢您的宝贵意见')));
            }
        }else{
            return $this->fetch();
        }
